...
3. **配置安全选项**：
   - **API Key**：访问管理后台 → 系统设置 → API Key。设置后，只有携带此 Key 的原生客户端（QuickDesk 桌面端）才能连接信令服务器，可防止未授权客户端接入
   - **Allowed Origins**（允许的 WebClient 域名）：访问管理后台 → 系统设置 → Allowed Origins。当 WebClient 单独部署在不同域名时（例如 `https://web.quickdesk.cc`），浏览器的同源策略会阻止 WebClient 跨域访问信令服务器。在此处添加 WebClient 的域名，信令服务器将对这些域名放行 CORS 请求。多个域名用逗号分隔
