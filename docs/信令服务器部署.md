...
后续如果需要修改配置，只需编辑 `.env.production` 文件，然后重新部署：
...
7. **客户端认证**：在 `.env` 中设置 `API_KEY` 限制原生客户端访问，设置 `ALLOWED_ORIGINS` 限制 WebClient 访问
...
- **管理后台**: `https://your-domain.com/admin/`（设备/用户/管理员账户/系统设置）
- **用户门户**: `https://your-domain.com/user-login`（用户注册、登录、设备绑定）
- **WebClient**: 独立部署，不包含在信令服务器中
