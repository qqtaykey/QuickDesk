...
graph TD
    ...
    subgraph MCP["quickdesk-mcp (Trust Layer)"]
...
sequenceDiagram
    ...
    participant MCP as quickdesk-mcp
    participant Risk as Risk Engine
    participant Qt as QuickDesk GUI
    participant User as User

    AI->>MCP: agent_exec(write_file, {path: "/etc/config"})
...
sequenceDiagram
    ...
    participant Qt as QuickDesk GUI
    participant MCP as quickdesk-mcp
    participant Agent as quickdesk-agent
    ...
    MCP->>Agent: send cancel signal
    Agent->>Skills: interrupt current operations
