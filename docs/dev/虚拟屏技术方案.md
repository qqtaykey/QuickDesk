...
graph TD
    subgraph ClientMachine["Client 机器（远程操控端）"]
        ...
        subgraph ClientProcess["quickdesk-client 进程"]
            QDClient["QuickDesk Client<br/>(视频解码 + 输入发送)"]
        end
        FloatingTB --> CM
    ...
    subgraph HostMachine["Host 机器（被控端）"]
        subgraph HostProcess["quickdesk-host 进程"]
            QDHost["QuickDesk Host<br/>(WebRTC 连接管理)"]
        end
        ...
        subgraph DriverLayer["IDD 驱动 (Session 0)"]
            IDD["quickdesk_display.sys<br/>IddCx Miniport"]
        end
...
```mermaid
graph TD
    Start["运行 QuickDesk 安装包<br/>(已具备 UAC 管理员权限)"]
    
    Start --> InstallApp["安装 QuickDesk 主程序"]
    InstallApp --> UserChoice{"用户勾选了<br/>'安装虚拟显示器驱动'?"}

    UserChoice -->|未勾选| SkipDriver["跳过驱动安装<br/>（虚拟屏功能不可用）"]
    UserChoice -->|勾选| CheckOldDriver{"检测旧版<br/>quickdesk_display 驱动?"}
...
graph LR
    ...
    subgraph Artifacts["产物"]
        Setup["QuickDesk-win-x64-setup.exe<br/>(含 drivers/vdd/)"]
        VDDArtifact["quickdesk-vdd artifact<br/>(驱动独立产物)"]
    end
